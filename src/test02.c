...
// example, 1d data (vector) with bool type
typedef bool mytype;
...
int main(void){
  ...
  mytype *data = NULL;
  // load
  reader("npyfiles/created-by-python-1d.npy", &ndim, &shape, &dtype, &is_fortran_order, &data);
  // dump
  writer("npyfiles/created-by-c-1d.npy",       ndim,  shape,  dtype,  is_fortran_order,  data);
  /* clean-up buffers */
  free(data);
  free(shape);
  free(dtype);
