...
// example, 3d data (array) with int type
typedef int mytype;
...
int main(void){
  ...
  mytype *data = NULL;
  // load
  reader("npyfiles/created-by-python-3d.npy", &ndim, &shape, &dtype, &is_fortran_order, &data);
  // dump
  writer("npyfiles/created-by-c-3d.npy",       ndim,  shape,  dtype,  is_fortran_order,  data);
  /* clean-up buffers */
  free(data);
  free(shape);
  free(dtype);
